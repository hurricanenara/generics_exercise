...
const myDiverseArray: MyDiverseArray<number | boolean> = [
  true,
  100,
  101,
  102,
...
const mixedArray: WithDefaultType<string | number> = [
  2023,
  "year of the rabbit",
];
...
export function strictCreateTupleTriplet<
  T extends string | number, //- T 의 타입을 제약 하면서 값도 종속?
  U,
  V extends [T]
...
//- 상속된 타입 변수
//- 기존에 사용하고 있는 타입을 상속해서 입력 받을 변수의 타입을 제한할 수 있다.
//- 참조 : https://hyunseob.github.io/2017/01/14/typescript-generic, https://www.typescriptlang.org/docs/handbook/2/generics.html#generic-constraints
//todo 제네릭 extends 대해 공부 필요
...
export function getRoleOptions(): Option<Role>[] {
  return Object.values(Role).map((item) => ({
    label: item
      .split("_")[1]
      .toLowerCase()
...
export function getCuisineOptions(): Option<Cuisine>[] {
  return Object.values(Cuisine).map((item) => ({
    label: `${item
      .split("_")[1]
      .toLowerCase()
...
interface IRepository<T, V> {
  create(data: T): void;
  findById(id: V): T | undefined;
  updateById(id: V): void | undefined;
  deleteById(id: V): void | undefined;
}
