...
const anotherDiverseArray: MyDiverseArray<string | object> = [
  myDiverseArray,
  "backend developer",
  { name: "hermione" },
];
...
type WithDefaultType<T = string | number> = T[];
...
const mixedArray: WithDefaultType = [2023, "year of the rabbit"];
...
export function getRoleOptions(): Option<Role>[] {
  return Object.values(Role).map((role) => {
    let lable = role.split("_")[1];
    return {
      label: lable[0] + lable.slice(1).toLocaleLowerCase(),
      value: role,
    };
  });
};
...
export function getCuisineOptions(): Option<Cuisine>[] {
  return Object.values(Cuisine).map((cuisine) => {
    let lable = cuisine.split("_")[1];
    return {
      label: lable[0] + lable.slice(1).toLocaleLowerCase() + " food",
      value: cuisine,
    };
  });
};
...
export class Queue<T> implements IQueue<T> {
  ...
  enqueue(item: T): void {
    if (this.size() === this.capacity) {
      throw Error("Be patient, the Queue is full");
    };
...
interface IRepository<T> {
  ...
  updateById(id: T, content: T): Promise<T | null>;
  deleteById(id: T): Promise<null | void>;
};
...
type EnumRecord<T extends string> = {
  [key in T]: T[];
};
