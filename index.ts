...
const anotherDiverseArray: MyDiverseArray<typeof myDiverseArray | string | Name> = [
    myDiverseArray,
    'backend developer',
    { name: 'hermione' },
];
...
export function strictCreateTupleTriplet<T, U, V extends T>(
    firstValue: T,
    secondValue: U,
    thirdValue: V[]
): [T, U, V[]] {
...
export function getRoleOptions(): Option<Role>[] {
    return Object.entries(Role).map(([label, value]) => {
        let labelFirstChar: string = label.split('_')[1].charAt(0);
        let labelOthers: string = label.split('_')[1].slice(1).toLowerCase();
        return {
            label: labelFirstChar + labelOthers,
            value,
        };
    });
}
...
export function getCuisineOptions(): Option<Cuisine>[] {
    return Object.entries(Cuisine).map(([label, value]) => {
        let labelFirstChar: string = label.split('_')[1].charAt(0);
        let labelOthers: string = label.split('_')[1].slice(1).toLowerCase();
        return {
            label: labelFirstChar + labelOthers + ' food',
            value,
        };
    });
}
...
interface IRepository<T> {
    create(item: T): void | Error;
    findById(id: number): T | Error;
    updateById(id: number): void | Error;
...
export function getLength<T extends string | number[]>(input: T): number {
    return input.length;
}
